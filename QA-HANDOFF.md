...
- **Dev-site HTTP Basic Auth is retrieved from sysPass at runtime** (v1.10). Compucorp dev/test sites sit behind a Traefik gateway with HTTP Basic Auth. The agent reads the credentials via `repro_helpers.get_syspass_cred(SITE_HOST, prefer_name="Basic HTTP Auth")` — this hits sysPass's JSON-RPC API using the `$SYSPASS_TOKEN_*` env vars (auto-injected by `start-symphony.sh`). The Mongo `compucorp.sites.<host>.basic_auth` value is Ansible-vault-encrypted and NOT used by the agent. The legacy `$DEV_SITE_BASIC_USER`/`$DEV_SITE_BASIC_PASS`/`$DEV_SITE_URL` env vars are honored if set but typically aren't. See `prompts/TOOLS.md` §"Compucorp dev sites" for the full pattern (including a safe Bash recipe with `-K` config to keep secrets out of `ps`).
