...
7. **No internal scaffolding in the PR body.** The PR goes to a Compucorp repo other engineers read. **Do NOT** mention: Symphony, the workflow file, "Phase A/B", workspace file paths (`~/symphony_workspaces/...`, `~/.claude/projects/...`), Jenkins job run numbers, internal tag names (`agent-<TICKET>-before`, `agent-<TICKET>-fix`), anondb hostnames used during dev-site provisioning, or any other internal orchestration detail. Keep the PR body indistinguishable from a competent human's PR.
   - Canonical failure: `compucorp/ies#240` `## Comments` listed Jenkins job names, run numbers, the `agent-IESBUILD-248-before` tag, and the anondb hostname — none of which are useful to a reviewer and all of which expose agent internals.
